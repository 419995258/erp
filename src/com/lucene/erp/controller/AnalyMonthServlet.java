...
/**
 * Servlet implementation class AnalyMonthServlet
 */
@WebServlet("/AnalyMonthServlet")
public class AnalyMonthServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AnalyMonthServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<User> userList = saleDao.queryAllDIANPU();
			if(!userList.isEmpty()){
				for (Iterator iterator = userList.iterator(); iterator.hasNext();) {
					...
					resultVo = saleDao.queryAnalyMonth(resultVo);
		...
		// 步骤3：创建饼状报表
		JFreeChart chart = ChartFactory.createPieChart3D(request.getParameter("message")+"月报表", dataset, true, true, true);
