...
/**
 * Servlet implementation class AnalyYearServlet
 */
@WebServlet("/AnalyYearServlet")
public class AnalyYearServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AnalyYearServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<User> userList = saleDao.queryAllDIANPU();
			if(!userList.isEmpty()){
				for (Iterator iterator = userList.iterator(); iterator.hasNext();) {
					...
					resultVo.setStrDiyDate(time);
					resultVo = saleDao.queryAnalyYear(resultVo);
					dataset.setValue(resultVo.getName(), resultVo.getMoney());
				}
			}
		// 步骤3：创建饼状报表
		JFreeChart chart = ChartFactory.createPieChart3D(request.getParameter("message")+"年报表", dataset, true, true, true);
		...
		message.put("1", graphURL);
		JSONObject json = new JSONObject();
		json.put("first", graphURL);
		out.print(json);
	}
