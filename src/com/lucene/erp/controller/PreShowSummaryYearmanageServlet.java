...
/**
 * Servlet implementation class PreShowSummaryYearmanageServlet
 */
@WebServlet("/PreShowSummaryYearmanageServlet")
public class PreShowSummaryYearmanageServlet extends PageServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public PreShowSummaryYearmanageServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 获取搜索字段
		Date Date = new Date();
		SimpleDateFormat sf=new SimpleDateFormat("yyyy");//设置转化格式
		...
		for (Iterator iterator = userList.iterator(); iterator.hasNext();) {
			...
			userId.put("user_id", user.getId());
			searchItem.put("user_id", user.getId());
			if (diyDate != null && !diyDate.isEmpty()){
				searchItem.put("DATE_FORMAT(t.diyDate,'%Y')", diyDate);
			}
			
			Statistic statistic = new Statistic();
			statistic = statisticDao.getYearPagingList(start, number, diyDate,searchItem);
			if(statistic.getUserId() != 0){
				int count = statisticDao.getCust(userId);
				statistic.setCustomerNum(count);
